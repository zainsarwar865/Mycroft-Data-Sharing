...
home_dir=/u/npatil/dvw/Cleanup/Mycroft-Data-Sharing/src
...
# MT Root parameters
enola_base_dir='/work/hdd/bdgs/npatil/test/dogsvwcleaned'
#enola_base_dir='/work/hdd/bdgs/npatil/test/optim'
mt_dataset="DogsVsWolves"
mt_config="MT_3"
mt_classes=10

MT_dataset_dir='/projects/bdgs/npatil/datasets/Imagenet_subsets/DF_zain'
do_dataset="DogsVsWolves"
do_config="DO_1"
DO_dataset_dir='/projects/bdgs/npatil/datasets/Imagenet_subsets/DF_zain'
# root_config --> subset
# Hash configs
...
if [ "$CREATE_ROOT" = true ]
then
    cd ${home_dir}/training/utils
    pwd
    # Path to config file 
    config_file="${home_dir}/training/utils/configs.py"
    # Check if the config file path is empty
    if [ -z "$config_file" ]; then
        echo "Error: No config file path provided for changing the path."
        exit 1
    fi
    echo "Updating dataset paths in $config_file..."
    # Call the Python script to update paths
    cd ${home_dir}/training/utils
    python3 update_configs.py "$config_file" "$MT_dataset_dir" "$mt_dataset" "$DO_dataset_dir" "$do_dataset"
...
# MT train parameters
batch_size=128  #change from food101
lr=0.1   #change from food101
weight_decay=1e-4   #change from food101
lr_warmup_epochs=0   #change from food101
lr_warmup_decay=0   #change from food101
label_smoothing=0   #change from food101
mixup_alpha=0  #change from food101
cutmix_alpha=0   #change from food101
random_erasing=0   #change from food101
model_ema=False 
epochs=20   #change from food101
num_eval_epochs=1
resume=False
finetune=False
pretrained=True
freeze_layers=True  #change from food101
seed=$seed
num_classes=2  #change from food101
new_classifier=True
...
# DHard parameters
bottom_k=2    #change from food101
per_class_budget=32   #change from food101
...
pretrained=True
freeze_layers=False
num_classes=10 # 120 to match hash configs   #change from food101
new_classifier=True
external_augmentation=False
test_per_class=True
original_dataset=$do_dataset
original_config="$do_config"
model="resnet50"
trainer_type="DO"
...
# Per class budget
d_hard_budget=100
num_candidates=32  #change from food101

# Unicom parameters
top_k=200   #change from food101
...
per_class='True'
numclasses=10  #change from food101
model='resnet50'
lam2=2  #change from food101
...
then
    ...
    then
        ...
        cd ${home_dir}/algorithms/unicom/unicom
    ...
    then
        cd ${home_dir}/algorithms/GradMatch/cords/tutorial
        python3 OMP_e2e.py \
        --home_dir=$home_dir \
        --enola_base_dir=$enola_base_dir \
        --root_hash_config=$root_hash_config \
...
then
    ...
    then
        cd ${home_dir}/training/utils
        python3 construct_retrieved_unicom.py \
        --enola_base_dir=$enola_base_dir \
        --root_hash_config=$root_hash_config \
        --mt_hash_config=$mt_hash_config \
        --do_hash_config=$do_hash_config \
...
random_erasing=0.1
model_ema=False
epochs=30  #change from food101
num_eval_epochs=1
resume=False
finetune=False #change from food101
pretrained=True
freeze_layers=False
seed=$seed
num_classes=120
