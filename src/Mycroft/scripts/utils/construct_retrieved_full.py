...
full_config = args.construct_hash_config
full_config_hash = hashlib.md5(full_config.encode('UTF-8')).hexdigest()
args.construct_hash_config = full_config_hash 
expr_name = f"Full_{full_config_hash}_{do_config_hash}"

full_retrievals_folder = "Retrievals/Full/"
expr_path = os.path.join(mt_root_directory, full_retrievals_folder)
...
full_folder = "Full"
full_dir = os.path.join(mt_root_directory, full_folder)
full_retrievals_dir = os.path.join(mt_root_directory, full_retrievals_folder)
full_dataset_config =  f"df_{args.DO_dataset}_{args.DO_config}_{full_config_hash}_{full_config_hash}_{do_config_hash}.pkl"
full_dataset_path = os.path.join(full_dir, full_dataset_config)
full_retrievals_path = os.path.join(expr_dir, full_dataset_config)
...
df_useful.to_pickle(full_dataset_path)
df_useful.to_pickle(full_retrievals_path)
# Created random 
print(f"Created df_full_useful at {full_dataset_path}")