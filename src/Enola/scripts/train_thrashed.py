...
def main_worker(gpu, ngpus_per_node, args):
    ...
    df_thrash = df_original_train[df_original_train['class'].isin(thrash_classes)]
    df_og = df_original_train[~df_original_train['class'].isin(thrash_classes)]

    for cl in thrash_classes:
        df_thrash_cl = df_thrash[df_thrash['class'] == cl] 
        keep_n = int(len(df_thrash_cl) * args.thrash_rate)
        df_thrash_cl_keep = df_thrash_cl.sample(n=keep_n, random_state=42)
        frames = [df_og, df_thrash_cl_keep]
        df_og = pd.concat(frames)
    ...
    if args.test_per_class:   
            class_stats_epoch, class_predictions_epoch, class_per_label_predictions_epoch, class_masks_epoch = get_sample_stats_classwise(df_val, val_loader, model, criterion, args)
