...
parser.add_argument('--d_hard_budget', type=int)
# Enola args
...
parser.add_argument('--retrieve_diff_classes', type=str)



# Use config files
args = parser.parse_args()
if args.retrieve_diff_classes == "True":
    args.retrieve_diff_classes = True
    print("set to true")
else:
    args.retrieve_diff_classes = False
    print("set to false")
...
if args.voting_scheme == 'borda-count':
    ...
    # Remove indices for classes which are not in MT's model
    mt_classes = df_mt_val['class'].unique().tolist()
    train_classes = df_train['class'].unique().tolist()
    mutual_classes = list(set(mt_classes) & set(train_classes))
    mutual_labels = df_train[df_train['class'].isin(mutual_classes)]['label'].unique().tolist()
    
    p_label = pred_label.T
    mask = np.isin(p_label, mutual_labels)
    p_label = p_label.tolist()
    for i in range(len(p_label)):
        p_label[i] = [lab for j, lab in enumerate(p_label[i]) if mask[i][j]]

    p_index = pred_index.T
    p_index = p_index.tolist()
    for i in range(len(p_index)):
        p_index[i] = [lab for j, lab in enumerate(p_index[i]) if mask[i][j]]

    
    if not args.retrieve_diff_classes:
        dhard_labels = query_label[0]
        mask = []
        for i in range(len(p_label)):
            t_mask = [True if lab == dhard_labels[i] else False for lab in p_label[i]]
            mask.append(t_mask)
        # Remove non-same classes
        for i in range(len(p_label)):
            p_label[i] = [lab for j, lab in enumerate(p_label[i]) if mask[i][j]]
        for i in range(len(p_index)):
    ...
    # Select top-k from each dhard sample
    top_k_retrieved = []
    for i in range(len(p_index)):
        t_topk = p_index[i][0:topk]
        top_k_retrieved.append(t_topk)

    top_k_retrieved = [item for sublist in top_k_retrieved for item in sublist]
    d_useful = np.asarray(list(set(top_k_retrieved)))

df_retrieved = df_train.iloc[d_useful]
