...
yaml_file = os.path.join(expr_dir, "Config_joint_optimization.yaml")
with open(yaml_file, 'w') as yaml_out:
    yaml.dump(expr_config_dict, yaml_out)


def get_dataset(df_do_class, df_mt_class, transform: Callable, transform_train=None) -> Dict:
    if transform_train is None:
        transform_train = transform
    if args.dataset == 'enola_datasets':
        from dataset.enola_dataset_classwise import enola_dataset_classwise
        trainset = enola_dataset_classwise(df_do_class, transform_train)
        testset = enola_dataset_classwise(df_mt_class, transform)
...
def main():
    ...
    geometric_distances_matrices = {}
    if args.eval:
        model, transform_clip = unicom.load(args.model_name)
        model = model.cuda()
        model = WarpModule(model)
        
        # Run this thing in a for loop with per class dataframes for train and test stuff
        do_path = args.do_train_path
        dhard_path = args.dhard_path
        df_do = pd.read_pickle(do_path)
        df_mt = pd.read_pickle(dhard_path)
        mt_unique_classes = df_mt['class'].unique()

        for uni_class in mt_unique_classes:
            print("Processing classs : ", uni_class)
            df_mt_class = df_mt[df_mt['class'] == uni_class]
            df_do_class = df_do[df_do['class'] == uni_class]
            if len(df_do_class) == 0:
                print(f"Skipping class {uni_class} for it has no data in DO's trainset")  
            dataset_dict: Dict = get_dataset(df_do_class, df_mt_class, transform_clip)
            score = evaluation(model, dataset_dict,
                            args.batch_size, args.num_workers, uni_class, geometric_distances_matrices)
            if rank == 0:
        ...
        # Save geometric-med distances dict
        retrieved_config = f"{args.DO_dataset}_{args.DO_config}_geometric_distances.pkl"
        geometric_matrix_path = os.path.join(expr_dir, retrieved_config)

        with open(geometric_matrix_path, 'wb') as o_file:
            pickle.dump(geometric_distances_matrices, o_file)
    else:
        if rank == 0:
...
def get_metric(
    ...
    if gallery is None:
        ...
        query_arr = query.clone().detach().cpu().numpy()
        geo_med_query = geometric_median(query_arr)
        geo_med_query = np.expand_dims(geo_med_query, axis=0)
        geo_med_query = torch.tensor(geo_med_query, dtype=query.dtype,device=query.device)
        geo_med_query = normalize(geo_med_query)        
        distances = euclidean_distance(geo_med_query, key)
        distances = distances.detach().cpu()
        distances = torch.diag(distances[0])
        matrices[uni_class] = distances
        ...
        while not is_end:
            ...
            # Compute GM of the query vector
            # Repeat it along the axis and send to euclidean_distance()
            # Remove top-k and you only need results for first index 
            
            print("Type of query is ", type(query))
            print("Query is ", query[0])

            _, index_pt = euclidean_distance(query[idx:end], key, topk=top_k)
            print("The distances are shape:", _.shape )
            print("The actual distances are : ", _)
            index_np = index_pt.cpu().numpy()[:, 0:top_k]
            print("Index np's shape is : ", index_np.shape)
...
def evaluation(model: torch.nn.Module,
               dataset_dict: Dict, batch_size: int, num_workers: int, uni_class: str, matrices : dict):
    ...
    #TODO Rename this to test
        ...
        metric = get_metric(matrices, uni_class, x_test, y_test, x_train, y_train)
