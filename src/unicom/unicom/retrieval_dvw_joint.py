...
parser.add_argument("--dataset", default="dogs_vs_wolves", help="The dataset to load for training and evaluation.")
...
parser.add_argument("--df_retrieve_config", type=str, default="", required=True)
...
parser.add_argument("--retrieval_save_path", type=str, default="/bigstor/zsarwar/Unicom_Retrievals/")
parser.add_argument("--median_stats_config", type=str, default="")
...
def get_dataset(args, transform: Callable, transform_train=None) -> Dict:
    ...
    if args.dataset == 'dogs_vs_wolves':
        from dataset.dogs_vs_wolves import dogs_vs_wolves
        trainset = dogs_vs_wolves(args, "train", transform_train)
        testset = dogs_vs_wolves(args, "eval", transform)
        trainset.num_classes = trainset.nb_classes()
        return {"train": trainset, "test": testset, "metric": "rank1"}
    
    if args.dataset == 'imagenet_openimages':
        from dataset.imagenet_openimages import imagenet_openimages
        trainset = imagenet_openimages(args, "train", transform_train)
        testset = imagenet_openimages(args, "eval", transform)
        trainset.num_classes = trainset.nb_classes()
        return {"train": trainset, "test": testset, "metric": "rank1"}
...
def get_metric(
    ...
    if gallery is None:
        ...
        # We only care about particular labels   
        #print("Size of query_label is ", query_label.shape)
        indices = np.where((query_label[0] == 0) | (query_label[0] == 1))[0]
        sub_query_label = query_label[:, indices]
        #print("Size of sub_query_label is ", sub_query_label.shape)
        sub_pred_label = pred_label[:, indices]
        sub_pred_index = pred_index[:, indices]
        tot_samples = sub_query_label.shape[0] * sub_query_label.shape[1]
        rank_1 = np.sum(sub_query_label == sub_pred_label) / tot_samples
        print("Total samples are ", tot_samples)        
        rank_1 = float(rank_1)
        
        # Save predictions for retriever
        retrieval_save_filename = os.path.join(args.retrieval_save_path, args.df_retrieve_config) 
        retrieval_save_filename = retrieval_save_filename.replace("df_train_", "").replace(".pkl", "")
        torch.save(sub_query_label, retrieval_save_filename + "_query_label.t")
        torch.save(sub_pred_index, retrieval_save_filename + "_pred_index.t")
        torch.save(sub_pred_label, retrieval_save_filename + "_pred_label.t")
        """
        # Compute geometric median of D_Hard (query_matrix)
        query_mat = query.detach().cpu().numpy()
        median = geometric_median(query_mat)
        median = median.reshape(1, -1)
        
        # Compute l2 distance between each sample in D_train and the geo_median
        key_mat = key.detach().cpu().numpy()
        #median_mat = np.repeat(median, key_mat.shape[0], axis=0)
        #l2_distances = euclidean_distances(median_mat, key_mat)
        l2_distances = euclidean_distances(key_mat, median)
        # Save 
        geo_stats = {}
        #print(type(median))
        #print(median)
        #np_median = median[0].tolist()
        geo_stats['dhard_geometric_median'] = median
        geo_stats['l2_distances'] = l2_distances

        stats_out_path = os.path.join(args.retrieval_save_path, args.median_stats_config)
        with open(stats_out_path, 'wb') as o_file:
            pickle.dump(geo_stats, o_file)
        #return rank_1 * 100
        return 1
    else:
        query = query.cuda()
