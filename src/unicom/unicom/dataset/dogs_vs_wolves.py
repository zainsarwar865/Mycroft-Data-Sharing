...
class dogs_vs_wolves(Dataset):
    def __init__(self, args, mode, transform=None):
        ...
        if self.mode == 'train':
            ...
            test_config = self.df_config.replace("train", "test")
            test_path = os.path.join(self.root, test_config)
            #test_path = test_path.replace("train", "test")
            df_path = os.path.join(self.root, test_path)
