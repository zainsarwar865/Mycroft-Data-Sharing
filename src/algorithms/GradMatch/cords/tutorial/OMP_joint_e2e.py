...
import os
import sys
#sys.path.append("/u/npatil/New/Mycroft-Data-Sharing/src/GradMatch/cords")
#os.chdir("/u/npatil/New/Mycroft-Data-Sharing/src/GradMatch/cords")
import time
import numpy as np
import hashlib
import pickle
...
parser.add_argument('--enola_base_dir', default='/work/hdd/bdgs/npatil/New/Enola_Augmented/')
...
parser.add_argument('--JointOptimization_hash_config', type=str)
...
if args.joint_optimization:
    ...
    unicom_expr_name = f"Unicom_{unicom_config_hash}_{do_config_hash}"
    unicom_expr_path = os.path.join(mt_root_directory, unicom_retrievals_folder)
    unicom_expr_dir = os.path.join(unicom_expr_path, unicom_expr_name)
    retrieved_config = f"{args.DO_dataset}_{args.DO_config}_geometric_distances.pkl"
    geometric_matrix_path = os.path.join(unicom_expr_dir, retrieved_config)
    
    with open(geometric_matrix_path, 'rb') as i_file:
        geometric_matrix_dict = pickle.load(i_file)
...
gradmatch_config = args.JointOptimization_hash_config
gradmatch_config_hash = hashlib.md5(gradmatch_config.encode('UTF-8')).hexdigest()
args.JointOptimization_hash_config = gradmatch_config_hash 
expr_name = f"JointOptimization_{gradmatch_config_hash}_{do_config_hash}"
gradmatch_retrievals_folder = "Retrievals/JointOptimization/"
...
if args.per_class:
    ...
    for unique_class in unique_classes:
        if unique_class == 'Ice cream':
            print("skipping ice cream")
            continue
        ...
        dss_args = dict(model=model,
                        ...
                        geometric_matrix_dict=geometric_matrix_dict,
                        curr_class=unique_class,
                        if_convex=False)
    ...
    """"
    trainset, validset, testset = get_datasets(df_train, df_val, df_test, args.arch)
    # Creating the Data Loaders
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.trn_batch_size,
                                            shuffle=False, pin_memory=True)
    valloader = torch.utils.data.DataLoader(validset, batch_size=args.val_batch_size,
    ...
    # TODO change back to trainloader
    dataloader = GradMatchDataLoader(trainloader, valloader, dss_args, logger, 
                                    batch_size=args.model_eval_batch_size,
                                    shuffle=True,
                                    pin_memory=False)
    ...
    """
    train_time = 0
    #for epoch in tqdm(range(len(all_ckpts[args.start:args.end]), args.jump_ckpts)):
    for epoch in range(args.start, args.end, args.jump_ckpts):
        # Evaluating the Model at Regular Intervals
        evaluate_model(curr_best_acc)
        subtrn_loss = 0
        subtrn_correct = 0
