...
class GradMatchDataLoader(AdaptiveDSSDataLoader):
    ...
    def __init__(self, train_loader, val_loader, dss_args, logger, *args, **kwargs):
        ...
        assert "joint_optimization" in dss_args.keys(), "joint_optimization is a compulsory argument for GradMatch"
        assert "geometric_matrix_dict" in dss_args.keys(), "geometric_matrix_dict is a compulsory argument for GradMatch"
        assert "curr_class" in dss_args.keys(), "curr_class is a compulsory argument for GradMatch"
        assert "lam2" in dss_args.keys(), "lam2 is a compulsory argument for GradMatch"
        ...
        self.strategy = GradMatchStrategy(train_loader, val_loader, copy.deepcopy(dss_args.model), dss_args.loss, dss_args.eta,
                                          ...
                                          logger, dss_args.curr_class ,dss_args.valid, dss_args.v1, dss_args.lam, dss_args.eps, dss_args.lam2, dss_args.joint_optimization, dss_args.geometric_matrix_dict)
