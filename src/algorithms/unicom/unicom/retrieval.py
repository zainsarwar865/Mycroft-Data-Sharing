...
def get_dataset(dataset_name: str, transform: Callable, transform_train=None) -> Dict:
    ...
    if dataset_name == "sop":
        ...
        return {"train": trainset, "test": testset, "metric": "rank1"}
    elif dataset_name == 'dogs_vs_wolves':
        from dataset.dogs_vs_wolves import dogs_vs_wolves
        trainset = dogs_vs_wolves(root, "train", transform_train)
        testset = dogs_vs_wolves(root, "eval", transform)
        trainset.num_classes = trainset.nb_classes()
        return {"train": trainset, "test": testset, "metric": "rank1"}
    else:
        raise
...
def get_metric(
    ...
    if gallery is None:
        ...
        indices = np.where((query_label[0] == 13) | (query_label[0] == 14))[0]
...
def extract_feat(
    ...
    #TODO make sure this isn't incorrect because of PASS subset in the main Doggy_vs_Wolves style
