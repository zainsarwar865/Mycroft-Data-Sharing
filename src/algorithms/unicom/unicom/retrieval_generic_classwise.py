...
import hashlib
import sys
basepath = os.path.join(args.home_dir, 'training', 'utils')
...
def main():
    ...
    if args.eval:
        ...
        # Initialize matrices
        query_label_matrix = {}
        query_label_matrix[0] = None
        pred_index_matrix = {}
        pred_index_matrix[0] = None
        pred_label_matrix = {}
        pred_label_matrix[0] = None
        ...
        for uni_class in mt_unique_classes:
            ...
            score = evaluation(model, dataset_dict,
                            args.batch_size, args.num_workers, query_label_matrix,
                            pred_index_matrix, pred_label_matrix)
            if rank == 0:
        ...
        # Save matrix tensors here
        retrieved_config = f"{args.DO_dataset}_{args.DO_config}"
        retrieved_config_path = os.path.join(expr_dir, retrieved_config)
        torch.save(query_label_matrix[0], retrieved_config_path + "_query_label.t")
        torch.save(pred_index_matrix[0], retrieved_config_path + "_pred_index.t")
        torch.save( pred_label_matrix[0], retrieved_config_path + "_pred_label.t")        
...
def get_metric(
    ...
    if gallery is None:
        ...
        if isinstance(query_label_matrix[0], np.ndarray):
            query_label_matrix[0] = np.concatenate((query_label_matrix[0], query_label), axis = 1)
        else:
            query_label_matrix[0] = query_label

        if isinstance(pred_index_matrix[0], np.ndarray):
            pred_index_matrix[0] = np.concatenate((pred_index_matrix[0], pred_index), axis = 1)
        else:
            pred_index_matrix[0] = pred_index
    
        if isinstance(pred_label_matrix[0], np.ndarray):
            pred_label_matrix[0] = np.concatenate((pred_label_matrix[0], pred_label), axis = 1)
        else:
            pred_label_matrix[0] = pred_label
...
def evaluation(model: torch.nn.Module,
               dataset_dict: Dict, batch_size: int, num_workers: int, query_label_matrix,
                            pred_index_matrix, pred_label_matrix):
    ...
    #TODO Rename this to test
        ...
        metric = get_metric(query_label_matrix, pred_index_matrix, pred_label_matrix, x_test, y_test, x_train, y_train)
        return metric
    """
