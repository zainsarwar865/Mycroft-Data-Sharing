...
def evaluate_model(curr_best_acc):
    ...
    if ("tst_loss" in print_args) or ("tst_acc" in print_args):
        samples =0
        with torch.no_grad():
            for _, data in enumerate(testloader_eval):
                inputs, targets = data
...
parser.add_argument("--data_budget", type=int)
...
config = f'{expr_name}_pretrained-{args.pretrained}_frozen-{args.frozen}_model-{args.arch}_numclasses-{args.numclasses}_select-subset-every-{args.select_subset_every}_budget-{args.data_budget}_batchsize-{args.trn_batch_size}_model-train-batchsize-{args.model_train_batch_size}_start-{args.start}_end-{args.end}_jump_ckpts-{args.jump_ckpts}_perclass-{args.per_class}'

config_hash = (hashlib.md5(config.encode('UTF-8')))
extended_expr_name = expr_name + "_" + f'budget-{args.data_budget}_perclass-{args.per_class}'
...
if args.per_class:
    for unique_class in unique_classes:
        ...
        trainloader_eval = torch.utils.data.DataLoader(trainset, batch_size=args.model_train_batch_size,
                                        shuffle=False, pin_memory=True)
        
        testloader_eval = torch.utils.data.DataLoader(testset, batch_size=args.model_train_batch_size,
                                                shuffle=False, pin_memory=True)


        valloader_eval = torch.utils.data.DataLoader(validset, batch_size=args.model_train_batch_size,
                                            shuffle=False, pin_memory=True)
