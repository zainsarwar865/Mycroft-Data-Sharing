...
from ..helpers import OrthogonalMP_REG_Parallel, OrthogonalMP_REG, OrthogonalMP_REG_Parallel_V1, OrthogonalMP_REG_Parallel_V1_Joint_Optimization
...
class GradMatchStrategy(DataSelectionStrategy):
    ...
    def __init__(self, trainloader, valloader, model, loss,
                 ...
                 selection_type, logger, curr_class, valid=False, v1=True, lam=0, eps=1e-4, lam2=0, joint_optimization=False, geometric_matrix_dict=None):
        """
        Constructor method
        """
        ...
        self.geometric_matrix_dict = geometric_matrix_dict
    ...
    def ompwrapper(self, X, Y, bud):
        if self.device == "cpu":
            ...
            if self.joint_optimization:
                reg = OrthogonalMP_REG_Parallel_V1_Joint_Optimization(X, Y, self.curr_class, self.geometric_matrix_dict, nnz=bud,
                                                 positive=True, lam=self.lam,lam2=self.lam2,
    ...
    def select(self, budget, model_params):
        ...
        if diff > 0:
            pass
            # Do not pad with random samples to fill the budget
            """
