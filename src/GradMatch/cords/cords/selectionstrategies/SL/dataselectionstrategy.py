...
class DataSelectionStrategy(object):
    ...
    def compute_gradients(self, valid=False, perBatch=False, perClass=False):
        ...
        if isinstance(trainloader.dataset[0], dict):
            ...
            tot_batches_trainloader = len(trainloader)
            max_batches_per_loader = 3000
            curr_batch_idx = 0
            start = 0
            end = min(tot_batches_trainloader, max_batches_per_loader)

            while curr_batch_idx < tot_batches_trainloader and start < end:
                # Create new trainloader using subset
                l0_grads = None
                l1_grads = None
                subset_indices = list(range(start, end))
                subset_train_dataset = Subset(trainloader.dataset, subset_indices)
                subset_train_dataloader = DataLoader(subset_train_dataset, batch_size=trainloader.batch_size, shuffle=False, pin_memory=trainloader.pin_memory)

                for batch_idx, (inputs, targets) in tqdm(enumerate(subset_train_dataloader)):
                    inputs, targets = inputs.to(self.device), targets.to(self.device, non_blocking=True)
                    if curr_batch_idx == 0 or l1_grads == None:
                ...
                # Custom stuff
                start = end
                end= min(end + max_batches_per_loader, tot_batches_trainloader)
                # Store l0_grads and l1_grads into self.grads_per_elem
                if self.linear_layer:
                    if type(None) == type(self.grads_per_elem):
                        self.grads_per_elem = torch.cat((l0_grads, l1_grads), dim=1)
                    else:
                        l0_l1_grads = torch.cat((l0_grads, l1_grads), dim=1)
                        self.grads_per_elem = torch.cat((self.grads_per_elem, l0_l1_grads), dim=0)
                else:
                    if type(None) == self.grads_per_elem:
                        self.grads_per_elem = l0_grads
                    else:
                        self.grads_per_elem = torch.cat((self.grads_per_elem, l0_grads), dim=0)        
                torch.cuda.empty_cache()
        
        
        torch.cuda.empty_cache()
        """
        # Doing this per trainloader now
        if self.linear_layer:
            self.grads_per_elem = torch.cat((l0_grads, l1_grads), dim=1)
        else:
            self.grads_per_elem = l0_grads
