...
def OrthogonalMP_REG(A, b, tol=1E-4, nnz=None, positive=False, lam=1):
    ...
    for i in range(nnz):
        ...
        if len(indices) == 1:
            ...
            A_i = np.vstack([A_i, A[:, index]])
            # np.identity shifts to being a diagonal matrix with median distances
            x_i = lstsq(A_i.dot(A_i.T) + lam * np.identity(A_i.shape[0]), A_i.dot(b))[0]
            # print(x_i.shape)
            if positive:
                while min(x_i) < 0.0:
...
# Joint optimization based function
def OrthogonalMP_REG_Parallel_V1_Joint_Optimization(A, b, curr_class, geometric_matrix_dict, tol=1E-4, nnz=None, positive=False, lam=1, lam2=2, device="cpu"):
    '''approximately solves min_x |x|_0 s.t. Ax=b using Orthogonal Matching Pursuit
    Args:
      A: design matrix of size (d, n)
      b: measurement vector of length d
    ...
    Returns:
       vector of length n
    '''
    print("Processing class: ", curr_class)
    psi_matrix = geometric_matrix_dict[curr_class]
    psi_vector = torch.diagonal(psi_matrix, 0)
    ...
    for i in range(nnz):
        ...
        indices_psi.append(index.item())
        if len(indices) == 1:
            A_i = A[:, index]
            x_i = projections[index] / torch.dot(A_i, A_i).view(-1)  # A_i.T.dot(A_i)
            A_i = A[:, index].view(1, -1)
        else:
            # Construct psi_matrix
            psi_indices = psi_vector[[indices_psi]]
            psi_matrix = torch.diag(psi_indices).to(device)
            psi_matrix = (lam2) * torch.matmul(torch.transpose(psi_matrix, 1, 0), psi_matrix)
            A_i = torch.cat((A_i, A[:, index].view(1, -1)), dim=0) 
            temp = torch.matmul(A_i, torch.transpose(A_i, 0, 1)) + lam * torch.eye(A_i.shape[0], device=device) + psi_matrix
            x_i, _, _, _ = torch.linalg.lstsq(temp, torch.matmul(A_i, b).view(-1, 1))
            if positive:
                while min(x_i) < 0.0:
                    argmin = torch.argmin(x_i)          
                    indices_psi = indices_psi[:argmin] + indices_psi[argmin + 1:]
                    indices = indices[:argmin] + indices[argmin + 1:]
    ...
    x_i = x_i.view(-1)
    print("Selected weights are :", x_i)
